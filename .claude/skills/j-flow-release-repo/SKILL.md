...
4. `claude plugin tag --dry-run` — verifies `plugin.json`/`marketplace.json` agree on version before tagging (this is a validation-only step: this repo tags as `vX.Y.Z`, not the `{name}--v{version}` format `claude plugin tag` itself would create, so never drop `--dry-run` or add `--push` here)
5. `git tag vX.Y.Z && git push origin main --tags`
6. `gh release create vX.Y.Z --title "vX.Y.Z" --notes "$(sed -n '/## \[X\.Y\.Z\]/,/## \[/p' CHANGELOG.md | sed '$d')"` (`head -n -1` broken on macOS — use `sed '$d'`)
