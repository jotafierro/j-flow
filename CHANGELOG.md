...
- **`**Spec language:**` and `**Docs language:**` in `.specs/config.md`** — write spec prose and the flow's own dialogues in your own language while the artifact schema stays English. Both default to `en`, so existing projects are unaffected, and the common setup is spec prose in your language with `docs/` and the CHANGELOG left in English for outside contributors.
  What is frozen is enumerated in `references/language-contract.md`, derived by sweeping the plugin rather than assumed: every `meta.md` key **and value**, the gate block names and `[stale]`, the backlog symbols, `[NEEDS CLARIFICATION`, the Given/When/Then keywords, `AC-N`/`DD-N`, and every artifact section heading. The catch-all rule is "if a skill looks for it, it is not translated; if a human reads it, it is."
  The failure mode this guards against is silent: a translated heading raises no error, it just means `/j-flow-plan`, `/j-flow-review` and `/j-flow-finish` stop finding the section and work with less context than they think they have. So the section schema of every parsed artifact template is snapshotted in `tests/validate.js` — translating or renaming a heading turns `npm test` red.
...
### Fixed
- **A scaffolded repo with the e2e layer no longer fails CI on its first push.** `create-playwright` aborts before writing its scripts, so `apps/e2e` kept `npm init`'s `"test": "echo \"Error: no test specified\" && exit 1"` — and CI runs `pnpm test`, which turbo maps onto every package's `test` script. Playwright now has an `e2e` script, run as its own turbo task and its own CI step. The root `pnpm test` deliberately excludes it: that is the command `/j-flow-qa` invokes at every gate, and it must stay unit-tests-in-seconds rather than booting browsers. The task is named `e2e` rather than `test:e2e` because `@nestjs/cli` already generates `test:e2e` in `apps/api` for its supertest suite — one name, two suites, one of which needs MongoDB.
- **`pnpm install` no longer leaves the workspace unable to run anything.** `auto-install-peers=true` materializes `ws`'s optional native peers (`bufferutil`, `utf-8-validate`, arriving through `vite`); pnpm ≥10 won't run their install scripts unasked, and pnpm 11 rewrites `pnpm-workspace.yaml` with `set this to true or false` placeholders and then **fails every `pnpm run`** until a human resolves them — not just the install. Those two plus `unrs-resolver` (which arrives through the ESLint toolchain `@nestjs/cli` generates, so whenever `has_api`) are now pre-seeded as `false`, all three confirmed safe on a full live scaffold. Since which packages pnpm asks about depends on the dependency tree (a real project needed `@scarf/scarf` and `unrs-resolver` instead), the generated README and review-guide document the procedure: resolve to `false` unless something demonstrably breaks, and never reach for a blanket allow-all.
- **The scaffold no longer invents dependency versions.** Two layer references told the model to add ten dependencies "at real version ranges", so two runs of the same `PRODUCT.md` produced different `package.json` files with nothing recording the difference. They are written out as concrete ranges — and deliberately kept out of both the catalog (single consumer each) and the pinned-tool-versions table (whose review requires a live scaffold per row, which application dependencies don't earn).
- **`/j-flow-finish` now writes the `[FINISH]` block it was always supposed to write.** `gate-core.md` §"Advancing a gate" requires a `gate-context.md` block for every phase and lists finish among them, and the same file already described what that block summarizes — but **the block's name was declared nowhere**, so runs silently skipped it: a real finished feature carries six blocks, `finish_status: completed`, and no finish block. `gate-core.md` now enumerates all seven names canonically, and `/j-flow-finish`'s step is renamed from "Update meta.md" to "Advance the finish gate" and spells out all three stores — being named after one store is why the other was forgotten. Existing finished features are left as they are; the rule applies forward.
...
- `tests/validate.js` gains five guards for scaffold output that actually runs, four for the gate block vocabulary (bidirectional: a declared name with no consumer, and an undeclared `[SOMETHING]` appearing in a skill), ten for the language contract, five for the workflow modes and four for the catalog policy — a cataloged dependency declared with a literal version, a `catalog:` reference with no matching entry (which fails installs with `ERR_PNPM_CATALOG_ENTRY_NOT_FOUND`), a missing expected entry, and a package that runs `oxlint` without declaring it. 185 → 216 checks.

## [2.4.0] - 2026-08-08

### Changed
- **`gate-context.md` now holds current gate state instead of full history, and a new `gate-log.md` holds the history.** It used to accumulate one block per gate *approval*: a feature revised eleven times carried eleven `[TECHNICAL SPEC]` blocks, and roughly a dozen skills plus every dispatched agent re-read all of them at every gate. It now holds exactly one block per gate — the current one — in gate order; when a gate is re-approved the existing block moves verbatim to `gate-log.md`, and `/j-flow-reopen` likewise appends removed blocks there rather than dropping them. Measured on a real 40-block feature: **5,417 → 753 tokens** in the file everything reads, with all 34 superseded blocks intact in the log.
  This **retires the append-only invariant on `gate-context.md`**, so it is a behavior change, not a bugfix — external tooling that reads that file expecting full history should read `gate-log.md` too. The auditability moves rather than disappears: `gate-log.md` *is* strictly append-only, and only `/j-flow-finish` reads it (for the feature README's decision history). `[stale]` and "superseded" stay deliberately distinct: `[stale]` marks a block that is still current for its gate but invalidated upstream, so it stays put; superseded means a newer approval replaced it, so it leaves. No migration — `gate-log.md` is created lazily and its absence means "nothing superseded yet", which is also what lets in-flight features carry on untouched.
- **`technical-spec.md` reads are scoped to the sections each reader needs.** It is the largest per-feature artifact (~16k tokens on a large feature). Four of the six builder agents already scoped to their own layer's section; `j-flow-backend` had drifted to reading the whole file and `/j-flow-build` asked for all of it even though its Build Loop already scoped per layer when dispatching. `j-flow-backend` now names the Data/Service/API Layer sections plus the three cross-layer ones, and `/j-flow-build` reads Architecture Overview and Design decisions only. `/j-flow-plan`, `/j-flow-review` and `j-flow-reviewer` still read the whole spec — planning and cross-layer auditing need it — and a guard records that as deliberate.
- **`/j-flow-project` and `/j-flow-check` load only the mode a run uses.** Each mode moved into its own reference, the same pattern the scaffold uses. `/j-flow-check`'s default status mode stays inline, since extracting the most-run path would add a read rather than remove one. Measured: `project --update` 5,202 → 1,747 tokens; `/j-flow-check` with no flags 4,040 → 1,482.
...
- **Lint standardizes on one tool per official CLI instead of one global choice.** `apps/web`, `apps/admin`, `packages/ui`, `packages/domain`, `packages/api-client`, and `apps/cli` now all reconcile to a new `packages/config/oxlint.base.json` (Vite's own template already ships oxlint; the three layers with no official CLI opinion — `domain`, `api-client`, `cli` — get their first `lint` script at all). `apps/api` deliberately keeps the ESLint config NestJS's CLI generates — its type-aware rules and Prettier integration stay put; `packages/config/eslint.base.js` now generates **only if `has_api`**, since it was silently orphaned in every project without a backend once Vite moved to oxlint upstream.
- `pnpm-workspace.yaml` gains a `catalog` block (`playwright`, when `has_e2e`) so `apps/e2e` and `packages/ui`'s Playwright peer resolve to the same pinned version instead of drifting independently — verified drift in a real scaffold (`^1.49.0` vs `^1.61.1`, two separate Chromium downloads). `packageManager` pin bumped `pnpm@9.0.0` → `pnpm@11.20.0` (catalogs require ≥ 9.5).
- **Official CLIs (and this skill's own hardcoded tool versions) now run at a pinned, known-good version instead of `@latest`.** `@nestjs/cli@11.0.24`, `create-vite@9.1.2`, `storybook@10.5.7`, `create-playwright@1.17.139`, `turbo@2.10.9`; `pnpm-workspace.yaml`'s `catalog` now carries **both** `playwright` and `@playwright/test` keys (they're different npm packages — a catalog with only one silently fails to resolve the other). A "Pinned tool versions" table above Step 4 replaces the old `@latest` confirmation prompt; Step 4 now prints what will install instead of asking, and warns (without blocking) when the pins haven't been reviewed in 6+ months. This closes plans 039–041 at the root: `@latest` and "resolve the real version live" both mean the skill's own post-processing steps silently drift out of sync with whatever a CLI actually generates — caught three times (Vite's default lint tool, a Storybook CLI flag, pnpm 11's config format) before deciding to stop chasing a moving target.
- **`pnpm-workspace.yaml` gains `overrides` and `allowBuilds`; the root `package.json`'s `"pnpm"` field is gone.** pnpm 11 no longer reads `package.json > pnpm.overrides` at all (silently — `me`'s esbuild-for-Storybook-compatibility override was being dropped on every install) and pnpm ≥10 ignores dependency install scripts unless `allowBuilds` explicitly allows them (`ERR_PNPM_IGNORED_BUILDS` otherwise). Both live in `pnpm-workspace.yaml` now.
- GitHub Actions in `ci.yml` (this repo's own, and the template `/j-flow-scaffold` generates) pin by plain major-version tag (`actions/checkout@v7`, `actions/setup-node@v7`, `pnpm/action-setup@v6`, `subosito/flutter-action@v2.23.0`) instead of commit SHA. Deliberate reversal of part of the SHA-pinning decision from the audit-remediation work: SHA pins are immune to a tag being repointed, but this repo has no Dependabot config to automate SHA bumps, so every update meant a manual `gh api` lookup. Chose readability and low-friction updates over that extra protection.

**Verified live, not just by the static validator**: a throwaway workspace scaffold (`packages/config`, `packages/domain`, `apps/web` via real `create-vite@9.1.2`, `apps/e2e` via real `create-playwright@1.17.139`, pnpm 11.20.0) was actually installed, linted, type-checked, and built. Caught one real bug in the process — `apps/web/tsconfig.node.json` needs `"moduleResolution": "nodenext"` set explicitly alongside `"module": "nodenext"`; leaving it unset made it inherit the base's `"bundler"` value, which TypeScript rejects (`TS5109`) before Vite even runs. Fixed in `layer-web.md`.
...
### Fixed
- **A scaffolded project's `gate-context.md` was seeded with the retired append-only header.** `/j-flow-scaffold` writes that file for `01-infra-base` directly, and its header still read "Append-only. Each phase adds one block" after the one-block-per-gate change — so every newly scaffolded repo started with a header contradicting the mechanics the skills follow. `README.md` and `docs/FLOW.md` described the old behavior too. A validator guard now checks the narrative docs for this class of drift, since the existing guards only covered the reference file, the templates and `/j-flow-finish`, and prose regressions had been passing `npm test` unnoticed.
