...
- **`pnpm install` no longer leaves the workspace unable to run anything.** `auto-install-peers=true` materializes `ws`'s optional native peers (`bufferutil`, `utf-8-validate`, arriving through `vite`); pnpm ≥10 won't run their install scripts unasked, and pnpm 11 rewrites `pnpm-workspace.yaml` with `set this to true or false` placeholders and then **fails every `pnpm run`** until a human resolves them — not just the install. Both are now pre-seeded as `false`. Since which packages pnpm asks about depends on the dependency tree (a real project needed `@scarf/scarf` and `unrs-resolver` instead), the generated README and review-guide document the procedure: resolve to `false` unless something demonstrably breaks, and never reach for a blanket allow-all.
