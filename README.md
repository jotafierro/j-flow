...
```
/j-flow-project           # one-time setup (PRODUCT.md, DESIGN.md, .specs/config.md, agent memory, auto-triggers scaffold)
...
| Skill | Purpose |
|-------|---------|
| `/j-flow-project [--update] [--from FILE] [--from-design FILE]` | Define project — PRODUCT.md, DESIGN.md, `.specs/config.md` (workflow mode + languages), agent memory; first run auto-triggers scaffold |
...
| `.specs/config.md` | Per-project workflow config — `**Workflow mode:**` (`team`/`solo`: PRs or trunk-based) and `**Spec language:**` / `**Docs language:**` (write spec prose in your own language; headings, gate vocabulary and code stay English). Absent means all defaults. |
