...
Write `CLAUDE.md` at the project root. This file is read by Claude Code on every session — keep it factual and command-focused, no prose.
...
(Substitute `{project}` and `{Project Name}` with actual values.)
