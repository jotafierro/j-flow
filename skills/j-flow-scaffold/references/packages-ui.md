...
Note for whoever re-enables `addon-vitest` later (component/interaction tests in browser mode): declare its `playwright` peer as `"playwright": "catalog:"`, not a hardcoded version — it must resolve from the same `catalog.playwright` entry `apps/e2e` uses (see `layer-e2e.md`), or the two packages install two different Chromium builds. This isn't generated by default today, but re-adding it by hand without the catalog reference is exactly how the version drift this plan fixes happened in the first place.
