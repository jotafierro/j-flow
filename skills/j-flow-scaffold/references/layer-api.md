...
- **Reconcile to the catalog:** rewrite `"typescript"` (the CLI pins `^5.7.3`) and `"@types/node"` (`^24.0.0`) to `"catalog:"` — the latter only if the catalog carries that key (see its condition in `SKILL.md` Step 2). Leave everything else the CLI generated at its literal range: `jest`, `ts-jest`, `eslint`, `typescript-eslint`, `prettier` and the rest are single-consumer here, since this layer deliberately keeps ESLint while the rest of the workspace uses oxlint. **Do not add `oxlint` to this package** — it has no `oxlint` script to run.

  This layer is why the catalog's `typescript` entry is `^5.9.0` rather than the `~6.0.2` `create-vite` ships: a freshly generated NestJS 11 project **fails `nest build` under TypeScript 6.0.2** (verified live 2026-08-17) with `TS5011` — the common source directory of `tsconfig.build.json` requires an explicit `rootDir` — and `TS5101` — `baseUrl` is deprecated and errors without `"ignoreDeprecations": "6.0"`. Both are raised against the tsconfig the Nest CLI itself writes, so they can't be post-processed away without diverging from the generator. The catalog holds the whole workspace on 5.x until that changes.
