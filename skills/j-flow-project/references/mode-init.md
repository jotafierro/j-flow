...
### Step 2c: Determine workflow mode and languages

Ask both in **one** message — this is the only place either is decided, and the init
dialogue is long enough already:
```
...
And what language should be written in?
  spec language — prose inside .specs/ and the dialogues I print   (default: en)
  docs language — docs/, CHANGELOG.md, README.md                   (default: en)

Enter e.g. "solo, es, en", or press Enter for team + en + en.
```

Set `workflow_mode`, `spec_language` and `docs_language` from the reply; anything blank
or unrecognized takes its default (`team`, `en`, `en`).

See `${CLAUDE_PLUGIN_ROOT}/skills/j-flow-shared/references/workflow-modes.md` for what each
workflow value changes, and `references/language-contract.md` for exactly which text each
language field governs — both are canonical. Here, only Step 6b writes them down and
Step 9b acts on the workflow mode.

Keeping `docs_language` at `en` while `spec_language` is something else is the common case
and is not a mistake: specs are for whoever works the repo, while `docs/` and the CHANGELOG
are what an outside contributor reads first.
...
Substitute `{Project Name}` from `PRODUCT.md`, and the three field values with the
`workflow_mode`, `spec_language` and `docs_language` chosen in Step 2c. Write to
`.specs/config.md`.
